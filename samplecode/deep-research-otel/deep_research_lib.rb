...
class ResearchAgent
  ...
  def initialize(api_key:, config:, &connection_customizer)
    @api_key = api_key
    @config = config
    @conn = Faraday.new(url: API_BASE, request: { timeout: @config.timeout_sec }) do |f|
  ...
  def generate_topics(user_content)
    ...
    response = generate_content(@config.topic_generator_model_name, payload)
  ...
  def research_topics(user_content, topics)
    ...
    worker_count = [[1, @config.max_parallel_research].max, topics.length].min
  ...
  def research_one_topic(user_content, topic)
    ...
    response = generate_content(@config.topic_researcher_model_name, payload)
  ...
  def synthesize(user_content, researched_topics)
    ...
    response = generate_content(@config.research_synthesizer_model_name, payload)
    response_text(response).strip
  end

  def generate_content(model, payload)
    path = "#{API_VERSION_PATH}/models/#{model}:generateContent"
    ...
    raise GeminiApiError, "HTTP #{response.status} on #{model}: #{response.body}"
  rescue JSON::ParserError => e
    raise GeminiApiError, "Invalid JSON response on #{model}: #{e.message}"
  end
  ...
  def response_text(response)
    ...
    candidates.each do |candidate|
      ...
      (content["parts"] || []).each do |part|
        text = part["text"]
        chunks << text if text && !text.empty?
      end
    end
    chunks.join("\n").strip
  end
